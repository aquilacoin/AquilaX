...
#if defined(NDEBUG)
#error "Aquila cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uAQX) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (Params().NetworkID() == CBaseChainParams::TESTNET) {
        if (nHeight < 200 && nHeight > 0)
            return 5000 * COIN;
	else if (nHeight > 200 && nHeight <= 10000000000000) //PoW stage 0 coins per block till 200
            return 100 * COIN;
    }
 ...
 */
    ...
    bool static ProcessMessage(CNode * pfrom, string strCommand, CDataStream & vRecv, int64_t nTimeReceived)
    {
        ...
        if (strCommand == "version") {
            ...
            // AQX: We use certain sporks during IBD, so check to see if they are
            ...
            if (state.fShouldBan) {
                if (pto->fWhitelisted)
                    ...
                    if (pto->addr.IsLocal())
                        ...
                        CNode::Ban(pto->addr, BAQXeasonNodeMisbehaving);
                    }
                }
