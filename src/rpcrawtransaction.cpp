...
UniValue getrawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "     {\n"
            "       \"value\" : x.xxx,            (numeric) The value in AQX\n"
            "       \"n\" : n,                    (numeric) index\n"
...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "3. \"addresses\"    (string) A json array of AQX addresses to filter\n"
            "    [\n"
            "      \"address\"   (string) AQX address\n"
            "      ,...\n"
            "    ]\n"
            "\nResult\n"
            ...
            "    \"address\" : \"address\",  (string) the AQX address\n"
            "    \"account\" : \"account\",  (string) The associated account, or \"\" for the default account\n"
            "    \"scriptPubKey\" : \"key\", (string) the script key\n"
            "    \"amount\" : x.xxx,         (numeric) the transaction amount in AQX\n"
    ...
    if (params.size() > 2) {
        ...
        for (unsigned int inx = 0; inx < inputs.size(); inx++) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid AQX address: ") + input.get_str());
            if (setAddress.count(address))
                throw JSONRPCError(RPC_INVALID_PAQXMETER, string("Invalid parameter, duplicated address: ") + input.get_str());
...
UniValue createrawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx   (numeric, required) The key is the AQX address, the value is the AQX amount\n"
            "      ,...\n"
            "    }\n"
    ...
    for (unsigned int idx = 0; idx < inputs.size(); idx++) {
        ...
        if (!vout_v.isNum())
            throw JSONRPCError(RPC_INVALID_PAQXMETER, "Invalid parameter, missing vout key");
...
UniValue decoderawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"GJrYZjVFfoF2nWbE72H9E5K9rYfLz1Eyi7\"   (string) AQX address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
UniValue signrawtransaction(const UniValue& params, bool fHelp)
{
    ...
    if (params.size() > 3 && !params[3].isNull()) {
        ...
        if (mapSigHashValues.count(strHashType))
            ...
            throw JSONRPCError(RPC_INVALID_PAQXMETER, "Invalid sighash param");
    }
